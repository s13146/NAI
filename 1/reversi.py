...
class Reversi(TwoPlayerGame):
    """
    Każdy z dwóch graczy ma do dyspozycji pionki: jeden koloru białego, drugi czarnego. Początkowo na planszy znajdują się po dwa pionki każdego z graczy. Gracze układają na przemian pionki własnego koloru na wolnych polach planszy do momentu, aż plansza zostanie całkowicie zapełniona lub żaden z graczy nie będzie mógł wykonać dozwolonego ruchu.
    Dozwolony ruch to taki, w którym pionek jest ułożony na polu, które znajduje się w linii (poziomej, pionowej lub ukośnej) z innym pionkiem gracza wykonującego ruch, i na dokładnie wszystkich polach pomiędzy wybranym polem a tym pionkiem znajdują się pionki przeciwnika. Te pionki zostają po wykonaniu ruchu przejęte i zmieniają kolor na przeciwny.
    ...
    def possible_moves(self):
        """ Dozwolnone są tylko takie ruchy ktore prowadza do odwrocenia kolorow. 
    ...
    def scoring(self):
        """ Poczatkowo zwraca sie uwage na rozmieszczenie pionkow, nastepnie tylko na to jaka liczbe pionkow na planszy ma dany gracz.
        
        """

        if np.sum(self.board == 0) > 32:  # mniej niz polowa tablicy jest pelna
...
def pieces_flipped(board, pos, current_player):
    """
    Funkcja zwraca liste elementow do odwrocenia jesli gracz wykona wlasciwy ruch.
